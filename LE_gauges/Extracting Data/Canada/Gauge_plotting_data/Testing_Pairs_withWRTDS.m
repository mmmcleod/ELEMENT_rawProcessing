...
for i =[1:24,26:length(gauges_Q)]% extract all but york which we have
    ...
    pathToExtractedWRDTSWQ='C:\Users\Meghan McLeod\Dropbox\BASULAB_meghan\Proposal\Lake Erie Data processing\LE_gauges\Extracting Data\Canada\WRTDS\Output_wrtds\';
    this_WQ_wrtds_file = [pathToExtractedWRDTSWQ,char(this_WQ),'_Results.xlsx'];
    this_WQ_wrtds_table = readtable(this_WQ_wrtds_file);
    this_WQ_wrtds_date = this_WQ_wrtds_table.DailyOUTPUT_Date;
    this_WQ_wrtds_value = this_WQ_wrtds_table.DailyOUTPUT_ConcDay;
    ...
    hold on 
    plot(this_WQ_wrtds_date,this_WQ_wrtds_value,'r')
    legend('Q','WQ')
    title({[char(this_WQ)]})
...
saveas(gcf,['testing_pair_data_with_daily_WRTDS.png'])
