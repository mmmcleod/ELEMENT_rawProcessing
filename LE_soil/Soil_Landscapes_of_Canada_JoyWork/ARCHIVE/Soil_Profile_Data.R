...
for (k in 1:length(fname)){
  ...
  SOIL_ID_subbas = unique(SOIL_subbas$SOIL_ID)
  
  SOIL_subbas_data = data.frame(SOIL_ID_subbas)
  SOIL_subbas_data$AREA = NA
  SOIL_subbas_data$TOTDEPTH = NA
  SOIL_subbas_data$avgOC = NA
  SOIL_subbas_data$avgBD = NA
  
  # for each unique SOIL_ID
  for (i in 1:nrow(SOIL_subbas_data)){
    
    idx = which(SOIL_ID_subbas[i]==SOIL_subbas$SOIL_ID)
    SOIL_subbas_data$AREA[i]=sum(SOIL_subbas$AREA[idx])
    
    idx = which(SOIL_ID_subbas[i]==SOIL_layr$SOIL_ID)
  ...
  # st_write(SOIL_subbas,paste0("SOIL_",fnamek,".shp"))
  ...
  write.xlsx(SOIL_subbas_data,paste0('SOIL_subbas_data_',fnamek,'.xlsx'))
